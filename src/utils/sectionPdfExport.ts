...
export interface SectionPdfOptions {
  ...
  scale?: number;
  /** Overlap in px between page slices to prevent glyph clipping (default 8) */
...
export async function exportSectionsToPdf(
  ...
  const addCanvasAcrossPages = (canvas: HTMLCanvasElement) => {
    ...
    while (sourceY < canvas.height) {
      ...
      sourceY = reachedEnd
        ? canvas.height
        : Math.max(0, sourceY + sliceHeightPx - cfg.sliceOverlapPx);
  ...
  const renderSectionCanvas = async (section: HTMLElement) => {
    ...
    wrapper.style.width = `${cfg.renderWidth}px`;
    ...
    try {
      return await html2canvas(wrapper, {
        ...
        width: cfg.renderWidth + 8,
        windowWidth: cfg.renderWidth + 8,
      });
    } finally {
