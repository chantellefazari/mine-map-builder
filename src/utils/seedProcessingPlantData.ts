...
export async function seedProcessingPlantData(): Promise<SeedResult> {
  ...
  try {
    ...
    // Deduplicate by pid_tag (keep first occurrence)
  ...
  try {
    ...
    for (const ep of equipmentPrefixes) {
      ncRows.push({
        convention_type: "equipment_prefix",
        code: ep.prefix,
        meaning: ep.meaning,
        description: "",
        example: ep.example,
    ...
    for (const cs of componentSuffixes) {
      ncRows.push({
        convention_type: "component_suffix",
        code: cs.suffix,
        meaning: cs.meaning,
        description: "",
        example: cs.example,
    ...
    for (const is_ of instrumentationSuffixes) {
      ncRows.push({
        convention_type: "instrumentation_suffix",
        code: is_.suffix,
        meaning: is_.meaning,
        description: "",
        example: is_.example,
    ...
    for (const sp of specialPatterns) {
      ncRows.push({
        convention_type: "special_pattern",
        code: sp.pattern,
        meaning: sp.meaning,
        description: "",
        example: sp.example,
