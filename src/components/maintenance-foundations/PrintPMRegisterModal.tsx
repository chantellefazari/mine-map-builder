...
export const PrintPMRegisterModal = ({ isOpen, onClose, pms }: Props) => {
  ...
  const buildHtmlContent = () => {
    ...
    // Group by discipline
    const disciplines = ["Mechanical", "Electrical", "Ops", "Inspection"];
    ...
    const renderTable = (items: PMItem[]) => `
      <table class="pm-table">
        ...
        <tbody>${items.map((pm, i) => `<tr>
          ...
          <td style="text-align:center">${pm.dutyType || "—"}</td>
          <td style="text-align:center">${pm.estimatedDuration || "—"}</td>
    ...
    return `
      ...
      <div class="banner">
        <div class="banner-left">
          ...
          <div class="doc-title">Site PM Register</div>
          <div class="doc-subtitle">Complete Preventive Maintenance Task Register — Processing Plant</div>
        </div>
      ...
      <div class="stats-bar">
        ...
        <div class="stat"><span class="stat-value">${opsCount}</span><span class="stat-label">Ops / Inspection</span></div>
      ...
      ${Array.from(grouped.entries()).map(([disc, items]) => `
        <div class="area-section">
          <div class="area-header">${disc} — ${items.length} PMs</div>
          ${renderTable(items)}
        </div>
      `).join("")}
  ...
  const pdfStyles = `
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .freq-badge.freq-weekly { background: #dcfce7; color: #166534; }
    .freq-badge.freq-monthly { background: #dbeafe; color: #1e40af; }
    .freq-badge.freq-6-weekly { background: #fef3c7; color: #92400e; }
    .freq-badge.freq-quarterly { background: #fef3c7; color: #92400e; }
    .freq-badge.freq-6-monthly { background: #fee2e2; color: #991b1b; }
    .freq-badge.freq-annual { background: #fce7f3; color: #9d174d; }
    ...
    try {
      ...
      while (yOffsetPx < imgHeightPx) {
        ...
        if (ctx) {
          ...
          ctx.drawImage(fullCanvas, 0, yOffsetPx, imgWidthPx, sliceHeightPx, 0, 0, imgWidthPx, sliceHeightPx);
        }
      ...
      <DialogContent className="max-w-md" aria-describedby={undefined}>
        ...
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Generate and download a professional PDF document listing all preventive maintenance tasks with discipline breakdowns and frequency summary.
          </p>
