...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .title-page { page-break-after: always; display: flex; flex-direction: column; justify-content: center; align-items: center; min-height: 90vh; text-align: center; padding: 40mm 20mm; }
    .title-page h1 { font-size: 28px; font-weight: 800; color: #1a1a1a; letter-spacing: -0.5px; margin-bottom: 8px; }
    .title-page .subtitle { font-size: 14px; color: #666; font-weight: 400; margin-bottom: 40px; }
    .title-page .site-name { font-size: 16px; font-weight: 700; color: #b8860b; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 6px; }
    .title-meta { margin-top: 30px; border-top: 2px solid #e5e5e5; padding-top: 20px; display: grid; grid-template-columns: 1fr 1fr; gap: 12px 40px; text-align: left; font-size: 11px; }
    ...
    .summary-title { font-size: 16px; font-weight: 700; border-bottom: 3px solid #b8860b; padding-bottom: 6px; margin-bottom: 16px; }
    ...
    .breakdown-table th { background: #f5f5f5; text-align: left; padding: 8px 10px; font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.3px; border-bottom: 2px solid #ddd; }
    .breakdown-table td { padding: 7px 10px; border-bottom: 1px solid #eee; font-size: 10px; }
    ...
    .area-header { background: #1a1a1a; color: white; padding: 8px 12px; font-size: 12px; font-weight: 700; letter-spacing: 0.3px; margin-bottom: 0; }
    .discipline-header { background: #f0f0f0; padding: 5px 12px; font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 1px solid #ddd; display: flex; align-items: center; gap: 6px; }
    ...
    .pm-table th { background: #fafafa; text-align: left; padding: 5px 8px; font-size: 8px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.3px; border-bottom: 1px solid #ddd; }
    .pm-table td { padding: 4px 8px; border-bottom: 1px solid #f0f0f0; font-size: 9px; vertical-align: top; }
    .pm-table tr { page-break-inside: avoid; }
    .pm-table tr:nth-child(even) { background: #fafafa; }
    .freq-badge { display: inline-block; padding: 1px 6px; border-radius: 3px; font-size: 8px; font-weight: 700; }
    ...
    .disc-badge { display: inline-block; padding: 1px 6px; border-radius: 3px; font-size: 8px; font-weight: 600; border: 1px solid #ddd; }
    .asset-match { color: #666; font-size: 8px; }
    .pid-tag { color: #2563eb; font-family: monospace; font-size: 8px; }
    .page-footer { position: fixed; bottom: 0; left: 0; right: 0; padding: 4px 10mm; font-size: 7px; color: #aaa; border-top: 1px solid #eee; display: flex; justify-content: space-between; }
    .disclaimer { margin-top: 20px; padding: 10px 14px; background: #fffbeb; border: 1px solid #fde68a; border-radius: 4px; font-size: 8px; color: #92400e; }
  </style>
</head>
<body>
  ...
  <div class="summary-page">
    ...
    <table class="breakdown-table">
      <thead><tr><th>Area</th><th style="text-align:center">Mechanical</th><th style="text-align:center">Electrical</th><th style="text-align:center">Total PMs</th><th style="text-align:center">Est. Hours</th></tr></thead>
      <tbody>
        ${areas.map(a => {
          ...
          return `<tr><td>${a.area}</td><td style="text-align:center">${a.mechanical.length}</td><td style="text-align:center">${a.electrical.length}</td><td style="text-align:center; font-weight:600">${total}</td><td style="text-align:center">${hours}h</td></tr>`;
        }).join("")}
        <tr><td>TOTAL</td><td style="text-align:center">${areas.reduce((s, a) => s + a.mechanical.length, 0)}</td><td style="text-align:center">${areas.reduce((s, a) => s + a.electrical.length, 0)}</td><td style="text-align:center">${totalPMs}</td><td style="text-align:center">${totalHours}h</td></tr>
      </tbody>
    </table>
    <div class="disclaimer"><strong>Important:</strong> P&ID tags are only shown where verified against the site's source of truth database.</div>
  </div>
  ${areas.map(a => {
    const renderTable = (items: ShutdownPM[]) => {
      if (items.length === 0) return "";
      return `<table class="pm-table"><thead><tr><th style="width:3%">#</th><th>PM Name</th><th style="width:8%">Freq</th><th style="width:7%">Disc.</th><th style="width:6%">Hours</th><th>Asset Match</th><th style="width:12%">P&ID Tag</th></tr></thead><tbody>${items.map((pm, i) => `<tr><td style="text-align:center; color:#aaa">${i + 1}</td><td style="font-weight:500">${pm.name}</td><td><span class="freq-badge freq-${pm.frequency.toLowerCase()}">${pm.frequency}</span></td><td><span class="disc-badge">${pm.discipline === "MS" ? "Mech" : "Elec"}</span></td><td style="text-align:center; font-family:monospace">${pm.estimatedHours}</td><td class="asset-match">${pm.tcAssetMatch || "—"}</td><td class="pid-tag">${pm.tcPidTag || "—"}</td></tr>`).join("")}</tbody></table>`;
    };
    return `<div class="area-section"><div class="area-header">${a.area}</div>${a.mechanical.length > 0 ? `<div class="discipline-header mech">■ Mechanical — ${a.mechanical.length} Items — ${a.mechanical.reduce((s, pm) => s + pm.estimatedHours, 0)}h Est.</div>${renderTable(a.mechanical)}` : ""}${a.electrical.length > 0 ? `<div class="discipline-header elec">■ Electrical — ${a.electrical.length} Items — ${a.electrical.reduce((s, pm) => s + pm.estimatedHours, 0)}h Est.</div>${renderTable(a.electrical)}` : ""}</div>`;
  }).join("")}
  <div class="page-footer"><span>TCMG-SD-PM-REQ-001 Rev 1.0</span><span>Tennant Creek Mining Group — Confidential</span><span>${today}</span></div>
</body></html>`;
  };
